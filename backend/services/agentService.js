...
export class AgentService {
  /**
   * Optional Gemini API Integration for rich LLM synthesis
   */
  static async callGeminiAPI(prompt, systemInstruction = '') {
    ...
    try {
      ...
      console.warn('Gemini API call warning (falling back to deterministic engine):', err.message);
      return null;
    }
  }

  /**
   * Process a natural language query with multi-step tool reasoning
   * @param {string} query 
  ...
  static async processQuery(query, user, history = []) {
    ...
    } else if (lowerQuery.includes('credit') || lowerQuery.includes('refund') || lowerQuery.includes('late') || lowerQuery.includes('delay')) {
      intent = 'SERVICE_CREDIT_INQUIRY';
    } else if (lowerQuery.includes('sla') || lowerQuery.includes('escalat') || lowerQuery.includes('breach') || lowerQuery.includes('response target') || lowerQuery.includes('p1') || lowerQuery.includes('p2') || lowerQuery.includes('p3')) {
      intent = 'SLA_AND_ESCALATION';
    } else if (lowerQuery.includes('bulk') || lowerQuery.includes('csv') || lowerQuery.includes('upload') || lowerQuery.includes('fail') || lowerQuery.includes('driver') || lowerQuery.includes('booked')) {
    ...
    // Handle Scenario 1: Cancellation Inquiry
    if (intent === 'CANCELLATION_INQUIRY') {
      ...
      if (targetOrder) {
        ...
        if (calcResult.canCancel && (lowerQuery.includes('cancel') || lowerQuery.includes('please cancel') || lowerQuery.includes('proceed'))) {
...
3. **Current Order State:** Order \`${targetOrder.order_id}\` has status **\`${targetOrder.status}\`** and has **not** been picked up by ${targetOrder.carrier}.
...
- **Governing Policy:** ${calcResult.governingAuthority}`;
        ...
        if (proposedAction) {
          answer += `\n\n> ⚠️ **State-Changing Action Prepared:** An action card has been generated below. Please click **Approve & Execute** to finalize this cancellation in the database.`;
        }
    ...
    // Handle Scenario 2: Service Credit Inquiry (Late Pickup)
    if (intent === 'SERVICE_CREDIT_INQUIRY') {
      ...
      if (targetOrder) {
        ...
        // If internal user asks to issue credit, prepare state action
        if (user.role !== 'customer' && (lowerQuery.includes('issue') || lowerQuery.includes('apply') || lowerQuery.includes('credit'))) {
          const actionPrep = ActionService.prepareAction(
...
4. **Approval Threshold:** INR 300 is below the INR 1,000 threshold requiring Ops Lead manager approval.
...
- **Governing Document:** ${creditCalc.governingAuthority}`;
        ...
        if (proposedAction) {
          answer += `\n\n> ⚠️ **State-Changing Action Prepared:** Click **Approve & Execute** below to issue the service credit to the order record.`;
        }
    ...
    // Handle Scenario 3: SLA & Ticket Escalation
    if (intent === 'SLA_AND_ESCALATION') {
      ...
      } else if (lowerQuery.includes('501') || lowerQuery.includes('shipment creation')) {
      ...
      } else if (lowerQuery.includes('505') || lowerQuery.includes('api key')) {
      ...
      if (targetTicket) {
        ...
        let targetMinutes = targetTicket.account_id === 'ACCT-001' ? 15 : (targetTicket.plan === 'Enterprise' ? 30 : 120);
        ...
        // State Action: Prepare Escalation if requested or P1 breach
...
**Recommended Action:** Immediate escalation to the on-call engineering lead and CSM Priya Mehta.`;
        ...
        if (proposedAction) {
          answer += `\n\n> ⚠️ **State-Changing Action Prepared:** Click **Approve & Execute** below to escalate Ticket \`${targetTicket.ticket_id}\` to the highest engineering priority queue.`;
        }
    ...
    // Handle Scenario 4: Product Operations / Known Issue Inquiry (e.g. Bulk CSV or Webhook)
    if (intent === 'PRODUCT_OPERATIONS_ISSUE' || lowerQuery.includes('bulk') || lowerQuery.includes('webhook') || lowerQuery.includes('driver')) {
      ...
      if (kiMatch.bestMatch && kiMatch.bestMatch.issueId === 'KI-208') {
        answer = `### ⚠️ Active Known Issue Identified: \`KI-208\` (Bulk Upload Failures on Large CSVs)

**Investigation Status:** \`Investigating\` (Opened: 10 August 2026)  
**Match Confidence:** **${kiMatch.bestMatch.matchScore}%**

#### 🔍 Root Cause Analysis:
- **Product Limit:** Growth and Enterprise plans officially support bulk uploads of up to **5,000 rows per CSV** (*Product Operations Guide Section 1*).
...
- **Bulk Upload:** Supported on **Growth** and **Enterprise** plans up to **5,000 rows per CSV**. (Not included on Standard plan).
- **Status Definitions:**
  - \`BOOKED\`: Shipment created; awaiting pickup confirmation.
  ...
  - \`DELIVERED\`: Final delivery complete.`;
    ...
    // Default: Knowledge Base Search + Gemini API Synthesis
    const docs = DocumentService.search(query, user);
    toolTrace.push({
      step: 3,
      tool: 'document_search',
      title: 'General Knowledge Base Search',
      details: `Retrieved ${docs.length} matching documents across policies and agreements`,
      status: 'COMPLETED'
    });
    ...
    if (topDoc) {
      citations.push({
        ...
        clause: 'General Guidance',
        confidence: 0.88
      });
    }

    // Attempt Gemini API synthesis if key is present
    let geminiSynthesis = null;
    if (CONFIG.GEMINI_API_KEY) {
      const systemPrompt = `You are the ParcelPilot AI Customer Support & Operations Agent. 
User: ${user.name} (${user.role}, Account: ${user.account_id || 'Internal Operations'}).
Precedence Rules: 1. Signed Customer Agreement > 2. Current Policy v3/SOP v4 > 3. Product Ops Guide > 4. Deprecated Policy v2 (quarantined) > 5. Historical Tickets (context only).
Reference Timestamp: ${CONFIG.REFERENCE_TIMESTAMP}.
Answer accurately based on the retrieved documents.`;

      const docsSummary = docs.slice(0, 3).map(d => `Document: ${d.title} (Tier ${d.authority_level})\n${d.content}`).join('\n\n---\n\n');
      const userPrompt = `Documents:\n${docsSummary}\n\nUser Question: ${query}`;
      
      geminiSynthesis = await this.callGeminiAPI(userPrompt, systemPrompt);
    }

    const answer = geminiSynthesis || `### 📦 ParcelPilot Intelligent Support Assistant

I analyzed your query: *"${query}"* across our active knowledge base and operational database.

**Summary of Relevant Policies & Operations:**
- **Signed Customer Agreements (Tier 1):** Customer-specific terms override general policies.
- **Support Policy v3 (Tier 2):** Defines standard P1/P2/P3 SLAs and immediate escalation protocols.
- **Cancellation & Service Credit SOP v4 (Tier 2):** Covers pre-pickup cancellations and failed pickup service credits.
- **Product Operations Guide (Tier 3):** Outlines plan capabilities (up to 5,000 rows bulk upload for Growth/Enterprise) and active known issues.
...
- *"What is the SLA status on ticket TKT-501?"*
- *"Why is the bulk CSV upload failing for 4,200 rows?"*`;
